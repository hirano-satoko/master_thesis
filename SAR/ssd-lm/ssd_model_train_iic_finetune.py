...
def add_noise(
    ...
    one_hot_value = args.hardcoded_pseudo_diralpha
    ...
    inputs_diralpha = 2 * one_hot_value * torch.nn.functional.one_hot(
        diffusion_input_ids, vocab_size
        ) - one_hot_value

    unit_noise = args.noise_manual_scale * one_hot_value * torch.normal(
        0, 1, size=inputs_diralpha.shape
        ).to(accelerator.device)
    ...
    return perturbed_inputs_simplex, selected_t
...
def train_one_epoch(
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        ## ContextとDiffusion入力を分割
        indexes = batch['index'][0] # [batch_size, 19]
        index_step = -1
        start_index, end_index = -1, -1
        while (start_index, end_index) == (-1, -1) or start_index>=args.max_seq_length:
            index_step = random.randint(0, 18)
            (start_index, end_index) = indexes[index_step]
        # end_index やら start_index　やらが最大しーけんす長を超える場合
        if end_index > args.max_seq_length:
            end_index = args.max_seq_length
        args.context_size = start_index + 1
        ...
        context_inputs_embeds = (
            model_embedding_lut(batch['input_ids'][:, :args.context_size])
            if args.context_size > 0
            else None
        )

        # ノイズの追加
        perturbed_inputs_simplex, selected_t = add_noise(
            diffusion_input_ids, 
            model,
            batch_size,
            vocab_size,
        ...
        timestep_embeds = timestep_layer(
            (selected_t / total_t).view(batch_size, 1, 1).repeat(1, seq_len, 1)
        )
        assert perturbed_inputs_embeds.size(1) == seq_len, f'perturbed_inputs_embeds shape is {perturbed_inputs_embeds.shape}, endindex{end_index}, startindex{start_index}, contextsize{args.context_size}, seqlen{seq_len}'
        ...
        equivalent_score = outputs.logits[:, args.context_size:].contiguous()
        ...
        ### classifier: batch[0]だけ処理
        assert batch['image_vectors'] is not None, "batch[image_vectors]には有効なデータが格納されていません。"
        assert batch['image_vectors'].shape[0] == outputs.hidden_states[-1].shape[0]
        image_feature = batch['image_vectors'][0] # [batch_size, 19, 768] の１番目なので [19, 768]
        language_feature = outputs.hidden_states[-1][0] # [層の数、batch_size, seqlen, hidden_size]で、結局[seqlen, hidden_size]
        classifier_outputs = classifier_layer(torch.mean(language_feature,dim=0)) # [768]
        classifier_loss = torch.nn.functional.mse_loss(input=classifier_outputs, target=image_feature[index_step].float())
        classifier_loss *= 10

        loss = model_loss + classifier_loss
        run.log({
            "train-loss": loss, 
            "train-model_loss": model_loss, 
            "train-classfier_loss":classifier_loss})
        ...
        # ログと保存
        if (step + 1) % (args.gradient_accumulation_steps * 10) == 0 or step == len(train_dataloader) - 1:
            ...
            if (step + 1) % (args.gradient_accumulation_steps * 100) == 0 or step == len(train_dataloader) - 1:
                ...
                if accelerator.is_main_process:
                    ...
                    torch.save(accelerator.unwrap_model(classifier_layer).state_dict(), os.path.join(args.output_dir, "classifier_layer.pt"))
                ...
                if (step + 1) % (args.gradient_accumulation_steps * _log_interval) == 0:
                    ...
                    if accelerator.is_main_process:
                        ...
                        torch.save(accelerator.unwrap_model(classifier_layer).state_dict(), os.path.join(ckpt_dir, "classifier_layer.pt"))
...
def validation(
    ...
    with torch.no_grad():
        for batch in val_dataloader:
            ...
            ## index
            indexes = batch['index'][0] # [batch_size, 19]の先頭なので [19]
            index_step = -1
            start_index, end_index = -1, -1
            while (start_index, end_index) == (-1, -1) or start_index>=args.max_seq_length:
                index_step = random.randint(0, 18)
            ...
            run.log({
                "valid-loss": loss, 
                "valid-model_loss": model_loss, 
                "valid-classfier_loss":classifier_loss})
...
def main():
    ...
    wandb.login()
    run = wandb.init(
        project="my-ssdlm-iic--project",
        name="20250203_segmented",
        config=config,
        )
    ...
    # Get the datasets
    train_dataset = load_data_text(
        tokenizer=tokenizer,
        split='train',
    )
    eval_dataset = load_data_text(
        tokenizer=tokenizer,
        split='valid',
    )
    ...
    classifier_layer = torch.nn.Linear(hidden_size, 768, bias=False)
    ...
    optimizer_grouped_parameters = [
        ...
        {
            "params": [p for p in classifier_layer.parameters()],
            "weight_decay": args.weight_decay,
        },
    ]
    ...
    model, embedding_sum_layer, timestep_layer, classifier_layer, optimizer, train_dataloader, eval_dataloader = accelerator.prepare(
        model, embedding_sum_layer, timestep_layer, classifier_layer, optimizer, train_dataloader, eval_dataloader
    )
    ...
    '''
    ###localized narrativesの場合
    train_dataset = 13,0831, batch_size=24, gpu=3
    len(train_dataloader) = 13,0831 / (24*3) = 1818
    args.gradient_accumulation_steps = 4 <- シェルスクリプト内で自分で調整
    num_update_steps_per_epoch = math.ceil(1818 / 4) = 455
    args.max_train_steps = 15,000 <- シェルスクリプト内で自分で調整
    args.num_train_epochs = math.ceil(15,000 / 455) = 32
    '''
    num_update_steps_per_epoch = math.ceil(len(train_dataloader) / args.gradient_accumulation_steps)
    args.num_train_epochs = math.ceil(args.max_train_steps / num_update_steps_per_epoch) # <- これがエポック数だよ多分
    ...
    if args.train_mode == "train" or args.train_mode == "resume":
        ...
        for epoch in range(args.num_train_epochs):
            model.train()
            completed_steps = train_one_epoch(
                model, 
                model_embedding_lut,
                embedding_sum_layer, 
                timestep_layer,
