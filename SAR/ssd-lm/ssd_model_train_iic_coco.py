...
from text_datasets_coco import load_data_text
import wandb
...
def train_one_epoch(
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        min_length = min(batch['length']).cpu().item()
        ctx_low, ctx_high = 2, min_length
        args.context_size = np.random.randint(low=ctx_low, high=ctx_high)
        seq_len = min_length - args.context_size + 2
...
def main():
    ...
    '''
    ###cocoの場合
    train_dataset = 566774, batch_size=24, gpu=3
    len(train_dataloader) = 566774 / (24*3) = 7872
    args.gradient_accumulation_steps = 8 <- シェルスクリプト内で自分で調整
    num_update_steps_per_epoch = math.ceil(7872 / 8) = 984
    args.max_train_steps = 10,000 <- シェルスクリプト内で自分で調整
    args.num_train_epochs = math.ceil(10,000 / 984) = 11
    '''
    num_update_steps_per_epoch = math.ceil(len(train_dataloader) / args.gradient_accumulation_steps)
    args.num_train_epochs = math.ceil(args.max_train_steps / num_update_steps_per_epoch) # <- これがエポック数
