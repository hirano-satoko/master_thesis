...
def main():
    ...
    if os.path.exists(args.model_name_or_path):
        ...
        _stdict = torch.load(os.path.join(args.model_name_or_path, "classifier_layer.pt"))
        _stdict = dict((_k[7:], _stdict[_k]) if _k.startswith("module.") else (_k, _stdict[_k]) for _k in _stdict)
        ...
        for epoch in range(args.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if args.loss_mode == "xe":
                    assert torch.all((diffusion_input_ids >= 0) & (diffusion_input_ids < vocab_size)), "Invalid label detected!"
                    
                    assert not torch.any(torch.isnan(equivalent_score)), "Output contains NaN values!"
                    assert torch.all(torch.max(equivalent_score, dim=-1)[0] < 50), "Output is too large!"
                ...
                wandb.log({"model_loss": loss, "classfier_loss":classifier_loss})
